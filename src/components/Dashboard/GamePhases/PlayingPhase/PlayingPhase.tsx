...
import SafeWinnerDisplay from './components/SafeWinnerDisplay';
...
function FixedPlayingPhaseView({
  currentGame,
  winners,
  soundEnabled,
  callDelay,
  ...
  const settings = currentGame?.settings || { prizes: undefined, callDelay: 5 };
  const gameState = currentGame?.gameState || { status: 'paused' };
  
  // Check for active prize configuration - with null safety
  ...
  // Determine game status based on gameState - with null safety
    ...
    <div className="space-y-6">
      ...
      {isGameComplete ? (
        ...
        <div className="space-y-4 sm:space-y-6">
          ...
          {currentGame?.numberSystem?.currentNumber && (
          ...
          <div className="bg-white rounded-lg shadow-sm border p-3 sm:p-4">
            <NumberBoard
              calledNumbers={currentGame?.numberSystem?.calledNumbers || []}
              queue={currentGame?.numberSystem?.queue || []}
              currentNumber={currentGame?.numberSystem?.currentNumber || null}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div>
            ...
            <p className="mt-2 text-3xl font-semibold text-gray-900">
              {currentGame?.numberSystem?.calledNumbers?.length || 0}/90
            </p>
          </div>
          <div>
...
export default FixedPlayingPhaseView;
