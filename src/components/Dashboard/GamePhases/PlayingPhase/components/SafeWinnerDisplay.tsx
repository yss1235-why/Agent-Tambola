...
// Define an empty default prizes configuration
...
export const SafeWinnerDisplay: React.FC<WinnerDisplayProps> = ({ 
  winners = {},
  tickets = {}, 
  bookings = {}, 
  prizes = DEFAULT_PRIZES,  // Provide default empty prizes config
  showAllPrizes = false,
  ...
  useEffect(() => {
    ...
    if (winners) {
      Object.entries(winners).forEach(([prizeType, ticketIds]) => {
        ...
        const isPrizeEnabled = prizes?.[prizeType as keyof typeof prizes] ?? false;
        
        // Skip prizes that aren't enabled
        if (!isPrizeEnabled) return;
...
export default SafeWinnerDisplay;
